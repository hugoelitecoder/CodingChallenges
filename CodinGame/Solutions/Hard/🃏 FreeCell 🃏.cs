...
class BitStream
{
    ...
    {
        return total <= 1 ? 1 : 32 - BitOperations.LeadingZeroCount((uint)total - 1);
    }
    ...
    public class Reader
    {
        ...
        public int Read(int total) { return (int)ReadBits(BitsNeededFor(total)); }
...
{
    private sbyte _size;
    public Foundation() { _size = 0; }
    public Foundation(Foundation other) { _size = other._size; }
    public int Size => _size;
    public void Push(Card card) { ++_size; }
    public bool Accepting(Card card) { return card.Rank == (Rank)_size; }
    public bool Has(Card card) { return _size >= 1 && card.Rank <= (Rank)(_size - 1); }
}
...
class Tableau
{
    ...
    {
        _index = other._index;
        _unsortedSize = other._unsortedSize;
        _sortedSize = other._sortedSize;
        _stack = other._stack;
        _top = other._top;
    }
    ...
    {
        var count = CountMovable(to);
        if (count > superMoveSize) count = superMoveSize;
        if (!to.IsEmpty()) to.PushTopToStack();
        if (count > 1)
            to._stack = (ushort)((to._stack << (count - 1)) | (_stack & ((1 << (count - 1)) - 1)));
        to._top = _top;
    ...
    public int CountMovable(Tableau target)
    {
        ...
        var lead = target.Top();
        var top = Top();
        var rankDiff = (int)lead.Rank - (int)top.Rank;
        if (rankDiff <= 0 || _sortedSize < rankDiff) return 0;
        if ((rankDiff & 1) == (top.Color == lead.Color ? 1 : 0)) return 0;
        return rankDiff;
    }
    public bool Accepting(Card card)
    {
        return IsEmpty() || card.IsBelow(Top());
    }
    ...
    private Card GetCard(int i)
    {
        ...
        var suit = isMajor ? (Suit)color : (Suit)(3 - (int)color);
        return new Card(suit, (Rank)((int)_top.Rank + depth + 1));
    }
    private void PushTopToStack()
    {
        _stack = (ushort)((_stack << 1) | (_top.IsMajor() ? 1 : 0));
    }
    private int RecountSorted()
    {
        if (_sortedSize > 0 || _unsortedSize == 0) return 0;
        if (Size() == 0) return 0;
        _top = InitTableau[_index][Size() - 1];
        _stack = 0;
        var numSorted = 1;
        while (numSorted < Size())
        {
            var i = Size() - numSorted;
            if (!GetCard(i).IsBelow(GetCard(i - 1))) break;
            numSorted++;
        }
        for (var i = Size() - numSorted; i < Size() - 1; ++i)
            _stack = (ushort)((_stack << 1) | (GetCard(i).IsMajor() ? 1 : 0));
        _sortedSize += (byte)numSorted;
        _unsortedSize -= (byte)numSorted;
        return numSorted;
    }
    public bool Equals(Tableau t)
    {
        if (_unsortedSize != t._unsortedSize || _sortedSize != t._sortedSize) return false;
        if (_sortedSize == 0) return true;
        if (_top != t._top) return false;
        return _stack == t._stack;
    }
}
...
readonly struct Move
{
    ...
    public Move(MoveType type, int from, int to = 0)
    {
        Type = type;
        From = (byte)from;
        To = (byte)to;
...
class FreecellNode : IBeamSearchNode<FreecellNode>
{
    ...
    private Tableau[] _tableaux;
    private Card[] _reserve;
    private byte _cardsUnsorted, _movesPerformed, _movesEstimated, _autoPlays;
    private uint _hash;
    private static List<uint> _reserveRand;
    private static List<List<uint>> _tableauUnsortedRand, _tableauSortedRand, _tableauTopRand;

    public FreecellNode()
    {
        _foundations = new Foundation[4];
        for (var i = 0; i < 4; i++) _foundations[i] = new Foundation();
        _tableaux = new Tableau[8];
        for (var i = 0; i < 8; i++) _tableaux[i] = new Tableau();
        _reserve = new Card[4];
    ...
    public void CopyFrom(FreecellNode other)
    {
        ...
        _cardsUnsorted = other._cardsUnsorted;
        _movesPerformed = other._movesPerformed;
        _movesEstimated = other._movesEstimated;
        _autoPlays = other._autoPlays;
        _hash = other._hash;
    ...
    public static void Initialize()
    {
        ...
        InitializeHashRand(52, out _reserveRand, rand);
        _tableauUnsortedRand = new List<List<uint>>(8);
        _tableauSortedRand = new List<List<uint>>(8);
        _tableauTopRand = new List<List<uint>>(8);
        for (var i = 0; i < 8; ++i)
        {
            InitializeHashRand(8, out var u, rand); _tableauUnsortedRand.Add(u);
            InitializeHashRand(14, out var s, rand); _tableauSortedRand.Add(s);
            InitializeHashRand(52, out var t, rand); _tableauTopRand.Add(t);
        }
    }

    private static void InitializeHashRand(int count, out List<uint> target, Random rand)
    {
        target = new List<uint>(count);
        for (var i = 0; i < count; ++i) target.Add((uint)rand.Next() << 16 | (uint)rand.Next());
    }
    ...
    public void ComputeHash()
    {
        ...
        for (var i = 0; i < 8; ++i)
        {
            _hash += _tableauUnsortedRand[i][_tableaux[i].UnsortedSize()];
            _hash += _tableauSortedRand[i][_tableaux[i].SortedSize()];
            if (!_tableaux[i].IsEmpty()) _hash += _tableauTopRand[i][_tableaux[i].Top().RawValue];
        }
    }
    ...
    private void EncodeMoves(List<FreecellNode> newNodes)
    {
        var index = 0;
        foreach (var node in newNodes) node.Moves.Write(index++, newNodes.Count);
    }

    private int FindFoundation(Card card)
    {
        var suit = (int)card.Suit;
        return _foundations[suit].Accepting(card) ? suit : -1;
    }
    private int MaxSuperMoveSize(int from, int to)
    {
        var freeCells = _reserve.Count(c => c.RawValue == -1);
        if (!_tableaux[to].IsEmpty()) return freeCells + 1;
        int emptyTableaus = 0;
        for (int t = 0; t < 8; t++)
            if (t != from && t != to && _tableaux[t].IsEmpty()) emptyTableaus++;
        return (freeCells + 1) * (1 << emptyTableaus);
    }
    private bool AllowReserveToFoundation() =>
        LastMove.Type != MoveType.ReserveToTableau && LastMove.Type != MoveType.TableauToTableau && LastMove.Type != MoveType.TableauToReserve;
    private bool AllowReserveToFoundation(int f) =>
        !((LastMove.Type == MoveType.ReserveToFoundation && f != LastMove.To)) &&
        !((LastMove.Type == MoveType.TableauToFoundation && f != LastMove.To));
    private bool AllowTableauToFoundation() => LastMove.Type != MoveType.ReserveToTableau;
    private bool AllowTableauToFoundation(int t, int f) =>
        !(LastMove.Type == MoveType.TableauToFoundation && LastMove.To != f && LastMove.From > t) &&
    ...
    {
        if (!_foundations[(int)card.Suit].Accepting(card)) return false;
        if (card.Rank <= Rank.R2) return true;
        var requiredRank = (Rank)((int)card.Rank - 1);
        if (card.Color == CardColor.RED)
            return _foundations[(int)Suit.SPADE].Has(new Card(Suit.SPADE, requiredRank))
                && _foundations[(int)Suit.CLUB].Has(new Card(Suit.CLUB, requiredRank));
        else
            return _foundations[(int)Suit.HEART].Has(new Card(Suit.HEART, requiredRank))
                && _foundations[(int)Suit.DIAMOND].Has(new Card(Suit.DIAMOND, requiredRank));
    }

    public FreecellNode ReserveToFoundation(int r, bool autoPlay = false)
    {
        var card = _reserve[r];
        _reserve[r] = new Card(-1);
        _foundations[(int)card.Suit].Push(card);
        _movesEstimated--;
        if (!autoPlay)
        {
            LastMove = new Move(MoveType.ReserveToFoundation, r, (int)card.Suit);
            _movesPerformed++;
        }
        return this;
    }
    ...
    public FreecellNode ReserveToTableau(int r, int t)
    {
        var card = _reserve[r];
        _reserve[r] = new Card(-1);
        _tableaux[t].Push(card);
        LastMove = new Move(MoveType.ReserveToTableau, r, t);
        _movesPerformed++;
        return this;
    }
    ...
    {
        var card = _tableaux[t].Top();
        _foundations[(int)card.Suit].Push(card);
        _cardsUnsorted -= (byte)_tableaux[t].Pop();
        _movesEstimated--;
        if (!autoPlay)
        {
            LastMove = new Move(MoveType.TableauToFoundation, t, (int)card.Suit);
            _movesPerformed++;
        }
        return this;
    }
    ...
    {
        var card = _tableaux[t].Top();
        _reserve[reserveSlot] = card;
        _cardsUnsorted -= (byte)_tableaux[t].Pop();
        LastMove = new Move(MoveType.TableauToReserve, t, reserveSlot);
        _movesPerformed++;
        return this;
    }
    ...
    {
        var newSorted = _tableaux[s].Move(_tableaux[t], MaxSuperMoveSize(s, t));
        _cardsUnsorted -= (byte)newSorted;
        LastMove = new Move(MoveType.TableauToTableau, s, t);
        _movesPerformed++;
        return this;
    }

    public bool StateEquals(FreecellNode n)
    {
        if (!_reserve.SequenceEqual(n._reserve)) return false;
        for (var f = 0; f < 4; ++f)
            if (_foundations[f].Size != n._foundations[f].Size) return false;
        for (var t = 0; t < 8; ++t)
            if (_tableaux[t].SortedSize() != n._tableaux[t].SortedSize() || _tableaux[t].Size() != n._tableaux[t].Size()) return false;
        for (var t = 0; t < 8; ++t)
            if (!_tableaux[t].Equals(n._tableaux[t])) return false;
        return true;
    }
}
...
class HashTable<TNode> where TNode : class, IBeamSearchNode<TNode>, new()
{
    ...
    public void Remove(TNode node)
    {
        ...
        var n = node.Next;
        if (p != null) p.Next = n;
        else _bins[node.Hash & _binMask] = n;
        if (n != null) n.Prev = p;
        Count--;
    }
}
...
class Beam<TNode> where TNode : class, IBeamSearchNode<TNode>, new()
{
    ...
    private List<Bucket<TNode>> _levels;
    private HashTable<TNode> _hashTable;
    private Pool<TNode> _pool = new Pool<TNode>();
    private readonly int _maxMoves;
    public Beam(int beamSize, int maxMoves)
    {
        _beamSize = beamSize;
        _maxMoves = maxMoves;
        _upperbound = maxMoves + 1;
    ...
    public TNode BeamSearch(TNode layout)
    {
        ...
        for (var i = 0; i < _levels.Count - 1; ++i)
        {
            ...
            if (newSolution != null)
            {
                if (solution != null) _pool.Delete(solution);
    ...
    private TNode CreateNewLevel(Bucket<TNode> curLevel, Bucket<TNode> newLevel)
    {
        ...
        curLevel.Iterate(node =>
        {
            ...
            foreach (var n in newNodesBuffer) n.ComputeHash();
            TNode found = ProcessNewNodes(newNodesBuffer, newLevel);
            if (found != null)
            {
                if (solution != null) _pool.Delete(solution);
                solution = found;
    ...
    private TNode ProcessNewNodes(List<TNode> newNodes, Bucket<TNode> newLevel)
    {
        ...
        {
            if (newNode.MinTotalMoves() >= _upperbound)
            {
                _pool.Delete(newNode);
                continue;
            }
            if (newNode.IsGoal() && newNode.MinTotalMoves() < _upperbound)
            {
                if (solution != null) _pool.Delete(solution);
                solution = newNode;
                _upperbound = solution.MinTotalMoves();
                continue;
            }
            ...
            else if (newNode.Bin() <= newLevel.Max)
            {
    ...
    public string EncodeSolution(TNode start, TNode finish)
    {
        ...
        for (var i = 0; i < finish.MovesPerformed; i++)
        {
            ...
            result.Append(node.LastMove.Encode());
        }
        _pool.Delete(node);
