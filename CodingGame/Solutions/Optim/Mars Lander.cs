...
class Player
{
    ...
    private const double HORIZONTAL_TIMESCALE_TUNING_FACTOR = 3.1;   // A tuning multiplier for the horizontal speed correction time.
    ...
    private const double HORIZONTAL_BRAKING_DEAD_ZONE_RATIO = 0.37;  // A ratio of max horizontal speed below which braking is ignored.
    private static double HORIZONTAL_BRAKING_SPEED_THRESHOLD = MAX_HORIZONTAL_SPEED * HORIZONTAL_BRAKING_DEAD_ZONE_RATIO;
    private const double FINAL_APPROACH_TIME_SECONDS          = 2.45;  // The time (s) from touchdown when the final landing flare begins.
    private static double ALTITUDE_THRESHOLD_FOR_FINAL_APPROACH = MAX_VERTICAL_SPEED * FINAL_APPROACH_TIME_SECONDS;

    private const double ENGINE_CUTOFF_VERTICAL_SPEED_RATIO = 0.81;  // Ratio of max vertical speed to determine when to cut engines.
