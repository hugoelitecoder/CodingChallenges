...
class Solution
{
    static void Main(string[] args)
    {
        ...
        var kLine = Console.ReadLine();
        var k = kLine != null ? int.Parse(kLine) : 0;
        var checkpoints = new List<string>();
        for (var i = 0; i < k; i++) checkpoints.Add(Console.ReadLine());
        var readTime = sw.ElapsedMilliseconds;
        sw.Restart();
        var game = new ConnectTheColours(h, w, rows, checkpoints);
        ...
        Console.Error.WriteLine($"[DEBUG] Components: {game.ColorCount} Colors, {game.UsableEmptyCount} Usable Tiles, {k} Checkpoints");
...
{
    public int H, W, Size, ColorCount, UsableEmptyCount;
    ...
    Dictionary<string, int> _colorToId;
    List<int> _colorOrder;
    ...
    int[] _visitedToken;
    int _bfsToken;
    public ConnectTheColours(int h, int w, string[] rows, List<string> checkpointData)
    {
        H = h; W = w; Size = H * W;
        _board = new int[Size];
        _checkpoints = new int[Size];
        _isVertical = new bool[Size];
        _isHorizontal = new bool[Size];
        Array.Fill(_checkpoints, -1);
        _colorNames = new List<string>();
        _colorToId = new Dictionary<string, int>();
        _endpoints = new Dictionary<int, List<int>>();
        _colorOrder = new List<int>();
        var usableEmpty = 0;
        for (var r = 0; r < H; r++)
        {
            var row = rows[r];
            for (var c = 0; c < W; c++)
            {
                ...
                _board[idx] = -1;
                if (ch == '.') usableEmpty++;
                else if (ch == 'X') _board[idx] = -2;
                else if (ch == 'V') { _isVertical[idx] = true; usableEmpty++; }
                else if (ch == 'H') { _isHorizontal[idx] = true; usableEmpty++; }
                else
                {
                    var id = GetColorId(ch.ToString());
                    _board[idx] = id;
                    if (!_endpoints.ContainsKey(id)) _endpoints[id] = new List<int>();
                    _endpoints[id].Add(idx);
                }
            }
        }
        foreach (var cp in checkpointData)
        {
            var parts = cp.Split(' ');
            var cId = GetColorId(parts[2]);
            _checkpoints[int.Parse(parts[1]) * W + int.Parse(parts[0])] = cId;
        }
        ColorCount = _colorNames.Count;
        for (int i = 0; i < ColorCount; i++) _colorOrder.Add(i);
        _colorOrder.Sort((a, b) => {
            var d1 = Manhattan(_endpoints[a][0], _endpoints[a][1]);
            var d2 = Manhattan(_endpoints[b][0], _endpoints[b][1]);
            return d1.CompareTo(d2);
        });
        UsableEmptyCount = usableEmpty;
        _currentEmpty = usableEmpty;
        _sw = new Stopwatch();
    ...
    int GetColorId(string s)
    {
        if (!_colorToId.ContainsKey(s)) { _colorToId[s] = _colorNames.Count; _colorNames.Add(s); }
        return _colorToId[s];
    }
    void InitializeNeighbors()
    {
        ...
        {
            if (_board[i] == -2) { for (int k = 0; k < 4; k++) _neighbors[i * 4 + k] = -1; continue; }
            int r = i / W, c = i % W, baseIdx = i * 4;
            int[] cands = { r > 0 ? i - W : -1, r < H - 1 ? i + W : -1, c > 0 ? i - 1 : -1, c < W - 1 ? i + 1 : -1 };
            int[] dirs = { 0, 0, 1, 1 };
            for (int d = 0; d < 4; d++)
            {
                int n = cands[d];
                bool v = true;
                if (n == -1 || _board[n] == -2) v = false;
                else
                {
                    bool vert = dirs[d] == 0;
                    if ((_isVertical[i] && !vert) || (_isHorizontal[i] && vert) || (_isVertical[n] && !vert) || (_isHorizontal[n] && vert)) v = false;
                }
                _neighbors[baseIdx + d] = v ? n : -1;
            }
        }
    }
    ...
    bool FindPath(int c, int curr, int target, int idx)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (n == -1) continue;
            if (n == target || (_board[n] == -1 && (_checkpoints[n] == -1 || _checkpoints[n] == c)))
            {
                int score = (n == target ? 0 : 1) << 28 | (_checkpoints[n] == c ? 0 : 1) << 24 | (GetDegree(n) & 0xFF) << 16 | Manhattan(n, target);
    ...
    bool IsDeadEnd(int p, int tgt, int c)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                int d = 0, nb = n * 4;
                bool check = _board[n] >= 0 || _checkpoints[n] != -1;
                for (int j = 0; j < 4; j++)
                {
                    int nn = _neighbors[nb + j];
                    if (nn != -1)
                    {
                        if (_board[nn] == -1) { if (_checkpoints[nn] == -1 || _checkpoints[nn] == _checkpoints[n] || _checkpoints[n] == -1) d++; }
                        else if (nn == tgt && _board[nn] == c) d++;
                        else if (_board[nn] >= 0) d++;
                    }
                }
                if (check && _board[n] >= 0) { if (d < 1) return true; }
    ...
    bool CheckFuture(int idx) { for (int k = idx; k < ColorCount; k++) { int c = _colorOrder[k]; if (!CanReach(_endpoints[c][0], _endpoints[c][1], c)) return false; } return true; }
    bool CanReach(int s, int e, int c)
    {
        ...
        while (head < tail)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    if (n == e || (_board[n] == -1 && (_checkpoints[n] == -1 || _checkpoints[n] == c))) { _visitedToken[n] = _bfsToken; _bfsQ[tail++] = n; }
                }
            }
        }
        return false;
    ...
    void AddSegment(int f, int t, int c) { int r1 = f / W, c1 = f % W, r2 = t / W, c2 = t % W; _resultSegments.Add($"{c1} {r1} {c2} {r2} {_colorNames[c]}"); }
}